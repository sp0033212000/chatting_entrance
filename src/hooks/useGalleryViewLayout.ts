...
export default function useGalleryViewLayout(participantCount: number) {
  ...
  const updateLayout = useCallback(() => {
    ...
    const containerWidth =
      containerRef.current.offsetWidth - GALLERY_VIEW_MARGIN * 2;
    ...
    while (maxVideoWidth - minVideoWidth > 1) {
      ...
      const isLower = layoutIsTooSmall(
        mid,
        participantCount,
        containerWidth,
        containerHeight
    ...
    setParticipantVideoWidth(
      newParticipantVideoWidth - GALLERY_VIEW_MARGIN * 2
    );
  }, [participantCount]);
